...
const ProfileHeader: React.FC<ProfileHeaderProps> = ({ profile }) => {
  ...
  return (
    <div className="flex flex-col md:flex-row items-center md:items-start gap-6 p-6 bg-gradient-to-b from-secondary/50 to-secondary/30 rounded-xl border border-secondary-foreground/20 backdrop-blur-md">
      <Avatar className="w-24 h-24 md:w-40 md:h-40 border-2 border-secondary-foreground/20">
        <AvatarImage src={photoUrl} alt={displayName} />
        <AvatarFallback className="bg-secondary-foreground/10 text-secondary-foreground text-2xl">{firstName?.charAt(0) || "?"}</AvatarFallback>
      </Avatar>
      <div className="flex flex-col gap-2 flex-grow text-center md:text-left">
        <h1 className="text-2xl md:text-4xl font-bold text-secondary-foreground">{displayName}</h1>
        <p className="text-md md:text-lg text-secondary-foreground/90">{headline}</p>
      ...
      {linkedInUrl && (
        <Button
          variant="outline"
          size="sm"
          className="flex items-center gap-2 mt-4 md:mt-0 bg-secondary-foreground/10 hover:bg-secondary-foreground/20 border-secondary-foreground/20 text-teal-400"
...
const ProfileSummary: React.FC<ProfileSummaryProps> = ({ profile }) => {
  ...
  return (
    <Card className="border border-secondary-foreground/20 bg-secondary/30 backdrop-blur-md text-secondary-foreground">
      <CardHeader className="pb-3">
        <CardTitle className="text-xl font-semibold text-secondary-foreground">About</CardTitle>
      </CardHeader>
      <CardContent>
        <div className="secondary-foregroundspace-pre-wrap text-secondary-foreground leading-relaxed">
          {summary}
        </div>
      </CardContent>
    </Card>
  ...
  return (
    <Card className="border border-secondary-foreground/20 bg-secondary/30 backdrop-blur-md text-secondary-foreground">
      <CardHeader className="pb-2">
        <CardTitle className="text-xl text-secondary-foreground">Experience</CardTitle>
      </CardHeader>
      <CardContent>
        <div className="flex flex-col gap-6">
          {positions.positionHistory.map((position: Position, index: number) => (
            <div key={index} className="flex gap-4">
              <Avatar className="w-12 h-12 rounded-md shrink-0 bg-secondary-foreground/10">
              ...
              <div className="flex flex-col gap-1 w-full">
                <div className="flex justify-between">
                  <h3 className="font-semibold text-lg text-secondary-foreground">{position.title}</h3>
                  {position.linkedInUrl && (
                    <a
                      href={position.linkedInUrl}
                      target="_blank"
                      rel="noopener noreferrer"
                      className="text-teal-400 hover:text-secondary-foreground transition-colors"
                    >
                      <ExternalLink size={16} />
                    </a>
                  )}
                </div>
                <p className="text-secondary-foreground">{position.companyName}</p>
                ...
                {position.description && (
                  <div className="mt-2 text-secondary-foreground secondary-foregroundspace-pre-wrap text-sm">
                    {position.description}
                  </div>
                )}
              </div>
...
const SkillsSection: React.FC<SkillsSectionProps> = ({ profile }) => {
  ...
  return (
    <Card className="border border-secondary-foreground/20 bg-secondary/30 backdrop-blur-md text-secondary-foreground">
      <CardHeader className="pb-2">
        <CardTitle className="text-xl text-secondary-foreground">Skills</CardTitle>
      </CardHeader>
      <CardContent>
        <div className="flex flex-wrap gap-2">
          {skills.map((skill: string, index: number) => {
            ...
            return (
              <span
                key={index} 
                className={`text-sm py-1.5 px-3 rounded-full border ${endorsementCount > 0 ? 'bg-blue-500/30 text-blue-300 border-blue-500/40' : 'bg-secondary-foreground/10 text-secondary-foreground border-secondary-foreground/20'}`}
              >
                {skill}
                {endorsementCount > 0 && (
